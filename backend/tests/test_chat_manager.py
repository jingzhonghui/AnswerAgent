...
class TestChatManager:
    ...
    def test_create_conversation(self, tmp_data_dir: Path):
        """创建对话应返回合法 UUID 并在磁盘生成文件"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
    ...
    def test_list_conversations(self, tmp_data_dir: Path):
        """创建多个对话后列表应返回全部"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
    ...
    def test_list_conversations_empty(self, tmp_data_dir: Path):
        """空数据目录应返回空列表"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
        assert mgr.list_conversations() == []

    def test_get_conversation(self, tmp_data_dir: Path):
        """获取对话详情应返回完整数据"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
    ...
    def test_get_conversation_not_found(self, tmp_data_dir: Path):
    ...
    def test_delete_conversation(self, tmp_data_dir: Path):
        """删除后文件应消失，列表不应包含"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
    ...
    def test_delete_conversation_not_found(self, tmp_data_dir: Path):
        """删除不存在的对话应抛出"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
        with pytest.raises(ConversationNotFoundError):
            mgr.delete_conversation(str(uuid.uuid4()))

    def test_rename_conversation(self, tmp_data_dir: Path):
        """重命名后 title 应更新"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
    ...
    def test_invalid_conversation_id(self, tmp_data_dir: Path):
    ...
    def test_path_traversal_after_validation(self, tmp_data_dir: Path):
        ...
        # 使用一个看似 UUID 但解析后超出 data_path 的 ID
        # 注意：这里 ID 本身是合法 UUID，需要看 resolve 后是否超出
        # 合法 UUID 不会超出目录，所以这个测试只验证 UUID 格式校验
        good_path = mgr._get_conversation_path(conv_id)
        assert str(good_path).startswith(str(tmp_data_dir))

    def test_append_user_message(self, tmp_data_dir: Path):
        """追加用户消息后应能在详情中看到"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
        conv_id = mgr.create_conversation()
    ...
    def test_append_assistant_message(self, tmp_data_dir: Path):
    ...
    def test_get_recent_history(self, tmp_data_dir: Path):
        """获取最近历史应返回正确的轮数"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
        conv_id = mgr.create_conversation()
    ...
    def test_empty_messages_history(self, tmp_data_dir: Path):
    ...
    def test_corrupted_json_skip_in_list(self, tmp_data_dir: Path):
        """损坏的 JSON 文件应在列表中被跳过"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
    ...
    def test_corrupted_json_get_raises_not_found(self, tmp_data_dir: Path):
    ...
    def test_atomic_write_safety(self, tmp_data_dir: Path):
        """原子写入不应产生残留临时文件"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
        conv_id = mgr.create_conversation()
    ...
    def test_multiple_kb_names(self, tmp_data_dir: Path):
    ...
    def test_create_default_title(self, tmp_data_dir: Path):
        """不传 title 时应使用默认标题"""
        mgr = ChatManager(data_path=str(tmp_data_dir))
        conv_id = mgr.create_conversation()
        conv = mgr.get_conversation(conv_id)
    ...
    def test_data_dir_created_automatically(self):
        """ChatManager 初始化时应自动创建数据目录"""
        with tempfile.TemporaryDirectory() as d:
            data_path = Path(d) / "nonexistent" / "subdir"
            mgr = ChatManager(data_path=str(data_path))
