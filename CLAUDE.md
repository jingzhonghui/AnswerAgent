...
AnswerAgent 被设计为一个基于本地文件目录的问答智能体：后端从本地知识库中检索相关文件内容，再通过 DeepAgents（`create_deep_agent()` + `astream_events(v3)`）调用 OpenAI 或 Anthropic 模型生成回答；前端通过 SSE 展示流式输出。

### 后端设计

后端目录为 `backend/app/`：

- `api/`：FastAPI 路由层，包含流式问答（`chat.py`）、外部问答（`external_chat.py`）、对话 CRUD、知识库列表、认证、管理接口。
...
- `core/agent_builder.py`：DeepAgents Agent 构建工厂，使用 `create_deep_agent()` 创建三种 Agent（通用问答、知识库问答、深度思考）。
- `core/agent_tools.py`：知识库工具封装（list_knowledge_bases / search_kb_files / read_kb_file），所有文件操作限制在 `KNOWLEDGE_PATH` 内。
- `core/chat_manager.py`：以 JSON 文件方式持久化对话。
- `models/schemas.py`：定义 Pydantic 请求和响应模型。

后端依赖包括 FastAPI、uvicorn、python-dotenv、pydantic-settings、deepagents、langchain、langchain-openai、langchain-anthropic、sse-starlette 和 jieba。
...
4. `agent_builder.py` 根据 `mode` 参数构建对应的 DeepAgent（`create_deep_agent()`），注入上下文、对话历史和当前问题。
5. 通过 `agent.astream_events(version="v3")` 消费流式事件，映射为 SSE 推给前端。
6. 深度思考模式（`mode=deep`）额外发出 `agent_think` / `agent_observe` 事件展示中间推理步骤。

### SSE 协议

流式接口为 `POST /api/chat/stream`，事件包括：

- `kb_matched`：返回匹配到的知识库名称列表。
- `files_selected`：返回某个知识库被选中的参考文件。
- `token`：返回流式生成内容片段。
- `agent_think`（deep 模式）：返回 Agent 推理/工具调用决策。
- `agent_observe`（deep 模式）：返回工具执行结果。
- `done`：返回回答完成事件和消息 ID。
- `error`：返回错误信息。
